...
Each CAN-Module can use up to 32 mailboxes, which can be configured as Rx- or Tx-mailboxes. 
...
Uint16 runProgram=1;										//Declaration of global variable
...
void main(void){
    ...
    while(runProgram){
        if (CAN_dataAvailableAtMailbox(CAN_A, 0)){           //Check if mailbox message available at CAN_A mailbox0
			CAN_get(CAN_A, 0, &msg, CAN_MB_0.messageLength); //Save message from CAN_A mailbox 0 in variable msg
			CAN_resetMailbox(CAN_A, 0);                      //Clear mailbox 0 at CAN_A
			...
			CAN_get(CAN_A, 1, &msg, CAN_MB_1.messageLength); //Save message from CAN_A mailbox 0 in variable msg
			CAN_resetMailbox(CAN_A, 1);                      //Clear mailbox 1 at CAN_A
        ...
        CAN_send(CAN_A, 2, &msg, CAN_MB_2.messageLength);	 //Send Data msg on Tx mailbox-number 2
    }
}
```
